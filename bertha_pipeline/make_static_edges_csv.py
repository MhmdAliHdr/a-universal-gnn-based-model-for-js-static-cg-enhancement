...
def outdegree(node, edges):
    ...
    for e in edges:
        if e[0] == node:
            deg += 1
    return deg
def make_static_edges(package):
    # Read the functions
    functions_df = pd.read_csv("/mansion/MH000070/bertha_pipeline/data/nodes/" + package + "_static_functions.csv", delimiter=" ", header=0, index_col=0)
    # The target static call graph
    package_path = "/mansion/MH000070/bertha_pipeline/raw_callgraphs/" + package + "_static_cg.json"
    # Opening the call graph
    file = open(package_path, "r")
    # Reading the json
    ...
    edges_csv = open("/mansion/MH000070/bertha_pipeline/data/edges/" + package + "_static_edges.csv", "w")
