...
# For each package
for pkg in packages_data[5000:]:
    data_file = open("/mansion/MH000070/repos_data.txt", "a")
    if (pkg[0] not in ["moo-color", "semver", "tslib", "chalk", "commander", "fs-extra", "debug", "typescript", "glob", "lodash", "yargs"]) and (pkg[1] != "https://github.com/DefinitelyTyped/DefinitelyTyped") and (re.findall("@", pkg[0]) == []):
        # Clone it
        try:
        ...
        # https://alexandra-zaharia.github.io/posts/kill-subprocess-and-its-children-on-timeout-python/. Retrieved on November 22, 2025
        try:
            sp = subprocess.run("/mansion/MH000070/jelly/lib/main.js -j /mansion/MH000070/bertha_pipeline/" + pkg[0] + "_static_cg.json" + " /mansion/MH000070/bertha_pipeline/packages/" + pkg[0], shell= True, cwd = "/mansion/MH000070/bertha_pipeline/packages", timeout=300)
        except:
        ...
        # https://alexandra-zaharia.github.io/posts/kill-subprocess-and-its-children-on-timeout-python/. Retrieved on November 22, 2025
        try:
            command = "docker run --entrypoint jelly --rm -v /mansion/MH000070/bertha_pipeline/packages/" + pkg[0] + ":/workspace -v /mansion/MH000070/bertha_pipeline" + ":/output -it jelly --npm-test /workspace -d /output/" + pkg[0] + "_dyn_cg.json.out"
        ...
        try:
            ...
            logging.warning("Failed to collect static functions for: " + pkg[0])
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
        ...
        try:
            ...
            logging.warning("Failed to collect dynamic functions for: " + pkg[0])
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
        ...
        try:
            ...
            logging.warning("Failed to collect static edges for: " + pkg[0] + ", reason: " + e)
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
        ...
        try:
            ...
            logging.warning("Failed to collect dynamic edges for: " + pkg[0])
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
        ...
        # Index the data
        try:
            p5 = Process(index_functions, args = (pkg[0]))
            p5.start()
            p5.join(timeout=300)
        except:
            logging.warning("Failed to index functions for: " + pkg[0])
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
        ...
        try:
            p6 = Process(turn_into_geo_graphs, args = (pkg[0]))
            p6.start()
            p6.join(timeout=300)
        except:
            logging.warning("Failed to generate torch geometric data for: " + pkg[0])
            # Save commit hash of the package if everything goes successfully and delete the package
            try:
                # Writing the information onto a file
                # Andreas, D., Caron, A., Çelik, S. H., cloud8bits, Correia, J., Dekker, F.,  Digio, Enderle, J. S., Graham, S.,  Isidore, J., Jerry T, Khan, A., Miller, M., Semnodime, Silver Light, static_rtti
